class AddDefaultHomeDisplayPreferencesForUser < ActiveRecord::Migration
  def self.up
    User.find_each do |user|
      ...
      user.preference.send( :create_top_section_prefs ) if !user.preference.new_record? && user.multi_valued_preferences.preference( :top_stories_cluster_group ).count.zero?
      user.save
    end
  end
